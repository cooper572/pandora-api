            ...
            if (nextUrl) {
                ...
                if (ct.includes('mpegurl') || ct.includes('m3u8') || nextUrl.includes('.m3u8')) {
                    ...
                    if (segUrl) {
                        ...
                        const segRes = await _originalFetch(segUrl, { method: 'HEAD', signal: AbortSignal.timeout(5000), headers: { 'User-Agent': getUA(), ...extraHeaders } });
                        segRes.body?.cancel();
                        if (!segRes.ok && segRes.status !== 206) return fail(`Segment failed: ${segRes.status}`);
                    }
                } else if (!nextRes.ok && nextRes.status !== 206) {
                    return fail(`Variant fetch failed: ${nextRes.status}`);
                }
            }
        }
