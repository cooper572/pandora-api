...
async function fetchServers(id, s, e) {
    ...
    try {
        const res = await fetch(`${BASE}${pagePath}`, { headers: { 'User-Agent': UA }, signal: AbortSignal.timeout(10000) });
        if (!res.ok) return null;
        const html = await res.text();
        const servers = [];
        const re = /<div[^>]+data-server="([^"]+)"[^>]*>.*?<span class="order-badge">(\d+)<\/span>/gs;
        let m;
        while ((m = re.exec(html)) !== null) servers.push({ name: m[1], order: parseInt(m[2], 10) });
