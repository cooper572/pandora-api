...
const HEADERS = { 'apikey': APIKEY, 'authorization': `Bearer ${APIKEY}`, 'content-type': 'application/json', 'origin': 'https://flaxmovies.xyz', 'referer': 'https://flaxmovies.xyz/' };

export async function getStream({ id, s, e }) {
    const isMovie = !s;
    const endpoint = isMovie ? `${API_BASE}/get-movie` : `${API_BASE}/get-tv`;
    const body = isMovie ? JSON.stringify({ id: String(id) }) : JSON.stringify({ id: String(id), season: Number(s), episode: Number(e) });
