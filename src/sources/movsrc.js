...
export async function getStream({ id, s, e }) {
    const url = s && e ? `${MOVSRC_API}/v1/tv/${id}/seasons/${s}/episodes/${e}` : `${MOVSRC_API}/v1/movies/${id}`;
    const res = await fetch(url, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/150 Safari/537.36', 'Accept': 'application/json' }, signal: AbortSignal.timeout(15000) });
    ...
    const result = { url: innerUrl, headers: innerHeaders };
    const allUrls = sources.map(src => {
        const rw = rewriteUrl(src.url);
        if (!rw) return null;
        const { url: u, headers: h } = extractFromProxyUrl(rw);
        return { url: u, headers: h };
    }).filter(Boolean);
    if (allUrls.length > 1) result.allUrls = allUrls;
    return result;
}