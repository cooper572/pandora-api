...
const SERVERS = [`${MOVIEBOX_URL}/moviebox`, `${API_URL}/holly`, `${API_URL}/air`, `${API_URL}/multi`, `${MOVIEBOX_URL}/net`, `${MOVIEBOX_URL}/bmb`];
const HEADERS = { Accept: 'application/json, text/javascript, */*; q=0.01', 'Accept-Language': 'en-US,en;q=0.9', Referer: `${BASE_URL}/`, Origin: BASE_URL };
const STREAM_HEADERS = { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36', Referer: `${BASE_URL}/`, Origin: BASE_URL };
const KEY_HEX = 'YThmMmExYjVlOWM0NzA4MTRmNmIyYzNhNWQ4ZTdmOWMxYTJiM2M0ZDVlM2Y3YThiOGNhZDFlMmQwYTRkNWM1Yg==';
const THIRD_PARTY_PROXY_PATTERNS = [/^https:\/\/[^/]+\.workers\.dev\/(?:m3u8|mp4)-proxy\?url=(.+?)(?:&|$)/, /^https:\/\/[^/]+\.workers\.dev\/((?:https?:\/\/|https?%3A%2F%2F).+)$/, /\/(?:m3u8|mp4)-proxy\?url=(.+?)(?:&|$)/];
...
async function decrypt(payload) {
    try {
        ...
        const iv = base64UrlToBytes(parts[0]), ciphertext = base64UrlToBytes(parts[1]), authTag = base64UrlToBytes(parts[2]);
...
function pickString(obj, keys) {
    for (const key of keys) { const val = obj[key]; if (typeof val === 'string' && val.trim()) return val.trim(); }
    return '';
}
...
function normalizeHeaders(raw) {
    ...
    const entries = Object.entries(raw).filter(([k, v]) => k.trim().length > 0 && v != null).map(([k, v]) => [k, String(v)]);
...
async function fetchServer(serverBase, id, s, e, ua) {
    const url = (!s || !e) ? `${serverBase}/movie/${id}` : `${serverBase}/tv/${id}/${s}/${e}`;
    const res = await fetch(url, { headers: { ...HEADERS, 'User-Agent': ua } });
    if (!res.ok) return null;
    let body = await res.json();
    if (body.isEncrypted && body.data) { body = await decrypt(body.data); if (!body) return null; }
...
export async function getStream({ id, s, e }) {
    ...
    const sorted = [...allSources].sort((a, b) => { const aIsHls = a.type === 'hls'; const bIsHls = b.type === 'hls'; if (aIsHls && !bIsHls) return -1; if (!aIsHls && bIsHls) return 1; return 0; });
    const primary = sorted[0];
    return { url: primary.url, headers: primary.headers, skipProxy: false, skipHlsCheck: true, allUrls: sorted.map(src => ({ url: src.url, headers: src.headers, skipProxy: false, skipHlsCheck: true })), subtitles: allSubtitles };
}