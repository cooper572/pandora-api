...
const HEADERS_BASE = { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/127.0.0.0 Safari/537.36', 'Accept-Language': 'en-US,en;q=0.9' };
...
async function searchLookMovie(type, title, year, clientIP) {
    ...
    for (const base of LM_DOMAINS) {
        try {
            const headers = { ...HEADERS_BASE, 'Accept': 'application/json', 'Referer': `${base}/`, 'X-Requested-With': 'XMLHttpRequest' };
            if (clientIP) headers['X-Forwarded-For'] = clientIP;
            const res = await fetch(`${base}/api/v1/${endpoint}/do-search/?q=${encodeURIComponent(title)}`, { headers, signal: AbortSignal.timeout(4000) });
            if (!res.ok) continue;
...
async function getPlayPageData(base, slug, type, clientIP) {
    ...
    try {
        const headers = { ...HEADERS_BASE, 'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8', 'Referer': `${base}/` };
        if (clientIP) headers['X-Forwarded-For'] = clientIP;
        const res = await fetch(`${base}/${path}/play/${slug}`, { headers, signal: AbortSignal.timeout(8000) });
...
async function getEpisodeId(html, s, e) {
        ...
        if (seasonMatch) {
            try {
                ...
                if (season) {
                    ...
                    const ep = Array.isArray(eps) ? eps.find(x => String(x.episode) === String(e)) : (eps?.[String(e)] || Object.values(eps || {}).find(x => String(x.episode) === String(e)));
    ...
    for (let i = 1; i < parts.length; i += 2) {
        ...
        if (epM && seM && String(epM[1]) === String(e) && String(seM[1]) === String(s)) return id;
    }
    try {
    ...
    const attrMatch = html.match(new RegExp(`data-season=["']${s}["'][^>]*?data-episode=["']${e}["'][^>]*?data-id=["'](\\d+)["']`, 'i')) || html.match(new RegExp(`data-episode=["']${e}["'][^>]*?data-season=["']${s}["'][^>]*?data-id=["'](\\d+)["']`, 'i'));
...
async function getStreams(base, type, streamId, hash, expires, clientIP) {
    ...
    const accessEndpoint = isShow ? `${base}/api/v1/security/episode-access?id_episode=${streamId}&hash=${hash}&expires=${expires}` : `${base}/api/v1/security/movie-access?id_movie=${streamId}&hash=${hash}&expires=${expires}`;
    try {
        const headers = { ...HEADERS_BASE, 'Accept': 'application/json', 'Referer': `${base}/`, 'X-Requested-With': 'XMLHttpRequest' };
        if (clientIP) headers['X-Forwarded-For'] = clientIP;
...
export async function getStream({ id, s, e, clientIP }) {
    try {
        ...
        const tmdbRes = await fetch(isTV ? `${TMDB_BASE}/tv/${id}?api_key=${tmdbKey}` : `${TMDB_BASE}/movie/${id}?api_key=${tmdbKey}`, { signal: AbortSignal.timeout(3000) });
        ...
        let streamId = null;
        if (isTV) streamId = await getEpisodeId(html, s, e); else streamId = match.id_movie || match.id || await getMovieId(html);
