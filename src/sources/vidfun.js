        ...
        try {
            ...
            const imports = {
                js_code: {
                    ...
                    'kotlin.wasm.internal.throwJsError': (msg, name, stack) => { const e = new Error(); e.message = msg; e.name = name; e.stack = stack; throw e; },
                    ...
                    'kotlin.wasm.internal.getCachedJsObject_$external_fun': (() => { const cache = new WeakMap(); return (obj, id) => { if ((typeof obj !== 'object' && typeof obj !== 'function') || obj == null) return id; const hit = cache.get(obj); if (hit === undefined) { cache.set(obj, id); return id; } return hit; }; })(),
...
async function syncTime() {
    ...
    try {
        const t0 = Date.now();
        const res = await fetch(`${NETWORK_BASE}/time`, { headers: { 'User-Agent': UA }, signal: AbortSignal.timeout(5000) });
        const t1 = Date.now();
        if (!res.ok) return;
...
async function buildScriptUrl(serverName, type, params) {
    ...
    const sortedRaw = Array.from(params.entries()).sort(([a], [b]) => a.localeCompare(b)).map(([k, v]) => `${k}=${v}`).join('&');
    const sign = await buildSign(path, sortedRaw);
    params.set('sign', sign);
    const sortedEnc = Array.from(params.entries()).sort(([a], [b]) => a.localeCompare(b)).map(([k, v]) => `${encodeURIComponent(k)}=${encodeURIComponent(v)}`).join('&');
...
async function getServers() {
    const res = await fetch(`${NETWORK_BASE}/servers`, { headers: { 'User-Agent': UA, 'Origin': 'https://vidfun.xyz', 'Referer': 'https://vidfun.xyz/' }, signal: AbortSignal.timeout(8000) });
    if (!res.ok) return [];
...
async function fetchStream(serverName, type, params) {
    ...
    const res = await fetch(url, { headers: { 'User-Agent': UA, 'Origin': 'https://vidfun.xyz', 'Referer': 'https://vidfun.xyz/' }, signal: AbortSignal.timeout(15000) });
...
function extractUrl(data) {
    ...
    for (const s of streams) {
        if (s.type === 'hls' && s.playlist?.startsWith('http')) return { url: s.playlist, headers: { 'Origin': 'https://vidfun.xyz', 'Referer': 'https://vidfun.xyz/' }, skipProxy: false };
        if (s.type === 'file' && s.qualities) {
            const sorted = Object.entries(s.qualities).sort((a, b) => parseInt(b[0]) - parseInt(a[0]));
            if (sorted.length) return { url: sorted[0][1].url, headers: { 'Origin': 'https://vidfun.xyz', 'Referer': 'https://vidfun.xyz/' }, skipProxy: false };
        }
        if (s.playlist?.startsWith('http')) return { url: s.playlist, headers: { 'Origin': 'https://vidfun.xyz', 'Referer': 'https://vidfun.xyz/' }, skipProxy: false };
    }
    return null;
}

export async function getStream({ id, s, e }) {
    try { await syncTime(); } catch { return null; }
    let servers;
    try { servers = await getServers(); } catch { return null; }
