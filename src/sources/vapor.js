export async function getStream({ id, s, e }) {
    try {
        const url = s && e ? `https://api.dmvdriverseducation.org/v1/tv/${id}/seasons/${s}/episodes/${e}` : `https://api.dmvdriverseducation.org/v1/movies/${id}`;
        ...
        if (!streamUrl && Array.isArray(data.sources)) streamUrl = data.sources[0]?.url || data.sources[0]?.file;
