...
async function fetchProviders() {
    if (cachedProviders && Date.now() - cacheTs < PROVIDER_CACHE_TTL) return cachedProviders;
    const res = await fetch(`${BASE}/providers`, { headers: { 'User-Agent': UA }, signal: AbortSignal.timeout(8000) });
...
function collectUrls(data) {
    ...
    for (const c of candidates) {
        ...
        out.push({ url: extracted.url, headers: Object.keys(extracted.headers).length ? extracted.headers : undefined, quality: c.quality || c.resolution || 'auto' });
    }
    return out;
}
...
export async function getStream({ id, s, e }) {
    ...
    await Promise.allSettled(
        providers.map(async (providerId) => {
            try {
                const url = isTV ? `${BASE}/tv/${id}/seasons/${s}/episodes/${e}/by/${providerId}` : `${BASE}/movies/${id}/by/${providerId}`;
                const res = await fetch(url, { headers: { 'User-Agent': UA }, signal: AbortSignal.timeout(10000) });
                if (!res.ok) return;
                const data = await res.json();
                for (const src of collectUrls(data)) allUrls.push(src);
            } catch { }
        })
    );
